...
There are slight differences in the methods used by Cytoscape and igraph for the
positioning of nodes and edges. As a result, visualizations of large networks 
or directed networks may look more comprehensible when made with Cytoscape. Since
plotting with igraph allows placing multiple visualizations in a grid, we 
recommend using igraph for the comparison of networks. 

For optimal results it is recommended to include a maximum of 100 nodes or 500 
edges for igraph visualizations, and 250 nodes or 2500 edges for Cytoscape 
visualizations.

The `anvis()` function offers some arguments that are only relevant to specific
output types. Arguments starting with "vis_" will affect both the visualizations
made with Cytoscape or igraph, the prefix "igr_" is used for arguments that are
only relevant for igraph visualizations, prefix "cyto_" arguments only affect
cytoscape visualizations, and prefix "netw_" arguments are relevant only for 
network exports.
...
range 0 to 1. This edge width is hardly noticeable in `r CRANpkg("igraph")` or 
[Cytoscape](https://cytoscape.org/) 
visualizations. Therefore, using the default (`vis_edge_factor = NULL`), 
will cause `anvis()` to use scaling values 3.25 for `r CRANpkg("igraph")` and 2 
for [Cytoscape](https://cytoscape.org/) 

In the below example, we show edges with a width that is doubled relative to the
default.

### Igraph

```{r}
# to show igraph edges about 2x wider than default, we choose 6.5
#   as a edge scaling factor
anvis(sepsis_net, vis_edge_factor = 6.5)
...
```{r}
# to show igraph edges 2x wider, we need to choose 4 as a edge scaling factor
#   for cytoscape
if (cyto_active) {
    anvis(sepsis_net, output_type = "cytoscape", vis_edge_factor = 4)
}
```

```{r, echo=FALSE, fig.cap="Visualization with increased edge width, made with Cytoscape. ", out.width = '100%'}
knitr::include_graphics("cyto_images/double_edge_w.png")
```

## Setting more node space

The spacing between nodes can be changed for visualizations made with Cytoscape 
(`output_type = "cytoscape"`), by using the 'cyto_node_space' argument. 
Visualizations made with `r CRANpkg("igraph")` are not affected by this argument,
because `r CRANpkg("igraph")` provides its own methods to choose layouts for 
nodes.

The use of 'cyto_node_space' is demonstrated below:

### Cytoscape

```{r}
# Make a visualization with increased spacing between nodes
if (cyto_active) {
    anvis(sepsis_net, output_type = "cytoscape", cyto_node_space = 2)
}
```

```{r, echo=FALSE, fig.cap="Visualization with increased node spacing, made with Cytoscape. ", out.width = '100%'}
knitr::include_graphics("cyto_images/more_nodespace.png")
```

## Visualization of multiple networks

`anvis` offers several options that facilitate network comparisons. For instance,
multiple networks can be shown in a grid with the same node layout for each 
network. Since [Cytoscape](https://cytoscape.org/) does not support arranging
visualizations in a grid, we recommend using `r CRANpkg("igraph")` for network
comparisons. 

To demonstrate visualizations of multiple networks, we first create a network 
list. Preparation of the networks can be done in a single step, since both 
`adjToNetwork` and `addVisAttr` can be used on multiple networks at once. 

```{r}
# We will show grouped nodes in the visualization, so we need to create a vector  
#   with grouping information. Since all adjacency matrices in the list (sepsis) 
#   contain the same nodes (column names) in the same order, we can make a 
#   single vector. If each network contained different nodes, or nodes in a 
#   different order, we would create a list of vectors with group labels, 
#   with one grouping vector for each network.
proteins <- colnames(sepsis[[1]])
groups <- dplyr::case_when(
    stringr::str_starts(proteins, "IL") ~ "group A",
    stringr::str_starts(proteins, "CCL") ~ "group B",
...
# Convert our list of adjacency matrices into a list of network objects, with
...
With `r CRANpkg("igraph")` as 'output_type', two options are available to show 
multiple network visualizations. The argument 'igr_grid' can be used to change 
between them, `igr_grid = FALSE` for showing networks separately or 
`igr_grid = TRUE` for showing them combined in a grid. Respectively, 'vis_save' 
will respond by saving multiple, or a single image. 

Below the first two networks of our list are shown separately:

```{r}
anvis(net_list[1:2], igr_grid = FALSE)
```

Next, showing the networks in a grid, this time for all networks:

```{r, fig.width=12, fig.height=10}
anvis(net_list, igr_grid = TRUE,
      igr_par_opts = list(mar=c(2,4,5,4)))
```

The 'igr_grid' argument can be used to manually specify grid dimension, we 
demonstrate this below:

```{r, fig.width=8, fig.height=12}
anvis(net_list, igr_grid = c(4,3),
      igr_par_opts = list(mar=c(2,4,5,4)))
```

Note that in the above visualizations we adjusted the margins of the networks 
using 'igr_par_opts', so the node labels are shown completely. More info on this
using this argument can be found in the 
[section on changing graphical parameters](#par_opts). 

Furthermore, the above visualizations show titles for the networks. These have been
extracted from the names of the network list. We can prevent that the names are 
shown by using `igr_grid_names = FALSE`. Alternatively, we can provide a separate 
vector with a name for each network.

```{r, fig.width=8, fig.height=4.5}
grid_titles <- c("case 1", "case 2")

anvis(net_list[1:2], igr_grid = TRUE, igr_grid_names = grid_titles,
      igr_par_opts = list(mar=c(4,4,4,4)))
```

### Cytoscape

Cytoscape does not support arranging network is a grid. Visualizing a list of 
networks in Cytoscape using `anvis()` will display each network in the Cytoscape
session if `vis_save = FALSE`, or will save each separately if `vis_save = TRUE`.

## Changing the layout of the nodes

By default `anvis` will use circular layouts for network visualizations. The 
order of the nodes in the circular arrangement is determined by their group and 
by the order in which they occur in the original data. 

### Arranging nodes by connectivity

Within the circular layout nodes can be rearranged by their connectivity. As 
demonstrated in the [section on adding network attributes](#node_size), the 
functions `adjToNetwork` or `addVisAttr`, can be used with `arrange_co = TRUE` 
to achieve this. If a list of networks is used with these functions, the 
nodes will be arranged by their connectivity averaged over all networks in the 
list. As a result, it becomes easy to detect the nodes that have, on average, 
the strongest connections to other nodes. In addition, the node positions will 
be identical for each visualized network, facilitating network comparison. 

#### Igraph

```{r, fig.width=8, fig.height=6}
# Create networks that have their nodes arranged by average connectivity
net_list2 <- adjToNetwork(sepsis[1:6],
                         node_attrs = "all",
                         edge_attrs = "all",
                         width_type = "partcor",
                         group_vec = groups, 
...
Note how larger nodes within each group are counter-clockwise 
respective to smaller nodes. The position of the nodes of the nodes in their group
is determined by the sum of the edges (connectivity) averaged over all networks. 

#### Cytoscape

Arranging by connectivity works in the same way as with igraph, but visualizations
will be made separate instead of in a grid. 

```{r}
# Make a visualization for the network that has its nodes arranged by 
#   average connectivity
if (cyto_active) {
    anvis(net_list2, output_type = "cytoscape")
}
```

Images of the output are omitted here, check igraph visualizations for reference.  

### Custom node layouts

By default, `anvis` will layout networks on a circle with the node labels 
arranged radially around this circle. This layout reduces overlap of nodes and 
node labels, and it ensures that the positions of nodes can be constant for 
easier network comparisons. Below we provide an instruction on how to use 
alternative layouts.

#### Igraph

The layout of networks made with `r CRANpkg("igraph")` can be changed with the
'igr_plot_opts' argument of `anvis()`. This argument takes a list with named 
elements as input. We can provide any of the igraph layout functions for the 
...
# We will use igraph::layout_as_grid for the layout of our network. This 
#  additional step allows us to alter the number of row and columns for this
#  layout function.
layout_func <- function(graph) igraph::layout_on_grid(graph, 
                                                      width = 4, 
                                                      height = 9)
# Use the grid node layout for 4 networks
#   We adjusted appearance of the node labels, aspect ratio of the node grid, 
...
The features of `anvis` visualizations can be combined with 
the layout options that are offered by the [Cytoscape](https://cytoscape.org/) 
software. For this, keep the Cytoscape session open after visualization with 
`anvis()`, and apply different layouts using the options offered by Cytoscape.

```{r}
# First create a visualization using anvis(), then apply a different layout in 
#   cytoscape
if (cyto_active) {
    anvis(sepsis_net, 
          output_type = "cytoscape", 
          vis_radial_labs = FALSE,
          cyto_close_session = FALSE)
}
```

```{r, echo=FALSE, fig.cap="Visualization with layout changed in Cytoscape.", out.width = '100%'}
knitr::include_graphics("cyto_images/alt_layout.png")
```

## Directed networks

Directed networks will be visualized by `anvis()` with edges that feature 
curvature and arrows. 
For network objects of class `"graphNEL"` or `"igraph"` the edge mode will be 
detected automatically, to make the appropriate visualization. Generally, we recommend 
using Cytoscape for creating visualizations of directed networks, as its way of
preventing overlap of directed edges generally creates more clear visualizations.

To demonstrate the visualization of directed networks with igraph and Cytoscape,
we generate some example data. 

```{r}
# Create an artificial directed network based on sepsis
dir_adj <- sepsis[[1]]
#   set half of our symmetric matrix to 0.
dir_adj[lower.tri(dir_adj)] <- 0
#   all proteins correlate perfectly with themselves, 
#     which is not informative. We draw random values
#     from the edges of the network for self interactions.
set.seed(11)
diag(dir_adj) <- sample(x = dir_adj[upper.tri(dir_adj)],
                        size = length(diag(dir_adj)),
                        replace = TRUE)

# Create directed network 
dir_net <- adjToNetwork(adj_mats = dir_adj,
                        directed = TRUE, self_loops = TRUE,
                        edge_attrs = "all", node_attrs = "all", 
                        width_type = "partcor", group_vec = groups, 
                        arrange_co = TRUE)
```

### Igraph

For `"graphNEL"` or `"igraph"` object the edge mode will be detected 
automatically. We could overwrite this behavior by setting the 'directed' 
argument to `FALSE`. 

```{r}
# Visualize directed network with igraph
anvis(dir_net)
```
...
```{r}
# Visualize directed network with Cytoscape
if (cyto_active) {
    anvis(dir_net, output_type = "cytoscape")
}
```

```{r, echo=FALSE, fig.cap="Visualization of directed network, generated with Cytoscape.", out.width = '100%'}
knitr::include_graphics("cyto_images/directed_network.png")
```

## Further customization

Some networks may require a few tweaks in addition to the automated 
visualization that is done by applying `anvis` functions with their default 
settings. For this, we aimed to integrate the flexibility that the underlying 
network visualization framework provides, as additional options of the `anvis()`
function. 

### Igraph

Visualization made with igraph make use of the igraph framework and the default
framework of R's graphical devices. Both of these can be conveniently controlled
with arguments of `anvis()`. 

#### Changing visual attributes

Users that are familiar with using `r CRANpkg("igraph")` will know of the many
arguments that can be used to customize visualizations. These options are 
available with the `anvis()` function through use of the 'igr_plot_opts' argument.
This argument expect input as a list of name = value pairs, and it will work 
with any argument that can be used with `plot.igraph` (check the options in the
documentation by executing `?igraph::plot.igraph`). Below we provide a few examples 
of adjustments that can be made using this argument. 

To change the color of node borders: 
...
To overwrite node sizes:

```{r}
anvis(sepsis_net, 
      igr_plot_opts = list(vertex.size = rep(c(4,17), times = 18)))
```

To change the aspect ratio of the visualization:

```{r}
anvis(sepsis_net, 
      igr_plot_opts = list(asp = 0.7))
```

Note: to change the node labels an different argument is available with the
`anvis()` function: 'igr_rad_lab_opts'. Again, this argument takes a input a 
list of name = value pairs. Check the documentation of `text()` by running 
`?text` to see all valid arguments names. We provide one example:

```{r}
anvis(sepsis_net, 
      igr_rad_lab_opts = list(col = "darkblue", cex = 1))
```

We can choose not to add radial labels, in which case we can use 'igr_plot_opts' 
to alter node label appearance.

```{r}
anvis(sepsis_net, 
      vis_radial_labs = FALSE,
      igr_plot_opts = list(vertex.label.color = "grey40"))
```

#### Changing options of `par()` {#par_opts}

`par()` provides many arguments to set graphical parameters. Changing the options
of the graphical parameters can be convenient, for example for adjusting the
margins of network visualizations, to ensure radial labels are show completely.
`par()` can be directly controlled with the argument 'igr_par_opts', which takes
a list of name = value pairs as input. Check the options that can be used with 
this argument by running `?par`.

```{r}
# reducing margins for demonstration
anvis(sepsis_net, igr_par_opts = list(mar=c(0,0,0,0)))
```


```{r}
# Changing the background
anvis(sepsis_net, igr_par_opts = list(bg = "grey20"),
      igr_rad_lab_opts = list(col = "white"))
...
To further customization of networks shown in Cytoscape can be done using the
`r Biocpkg("RCy3")` package. In the example below we show how the edge line 
types of a directed network can be changed with `r Biocpkg("RCy3")`.

```{r}
if (cyto_active) {
    # RCy3 allows automated control of Cytoscape networks
    library(RCy3)
    
    # We will change the line style of a directed network
    # Get the line style options
    getLineStyles()
    
    # Visualize directed network with Cytoscape
    anvis(dir_net, output_type = "cytoscape")
    
    setEdgeLineStyleBypass(edge.names = getAllEdges(),
                           new.styles = "DOT")
}
```

```{r, echo=FALSE, fig.cap="Linetype changed with RCy3.", out.width = '100%'}
knitr::include_graphics("cyto_images/changed_linetype.png")
```

## Saving {#saving}

`anvis()` supports saving of visualizations in the following formats: "png" 
(default), "jpeg", "pdf", "svg" and "ps". These can be chosen with the 
'vis_export_type' argument, and are compatible with both igraph and Cytoscape. 
Using 'vis_export_opts' we can specify further preferences for saving, for 
example by changing the resolution or size of the saved image. 

### Igraph

The following code demonstrates how to save an image with adjusted file type
and export options.
...
anvis(sepsis_net,
      vis_save = T, 
      vis_export_type = "svg", 
      vis_export_opts = list(width = 5, height = 5))
```

```{r, echo=FALSE, fig.cap="The exported svg image. ", out.width = '100%'}
knitr::include_graphics("igraph_images/saved_image.svg")
```
Note: The options provided to 'vis_export_opts' must match with the graphics 
device that is used for saving. For example `res = 600` may be valid for "png", 
but not for "svg". The options can be check in the documentation of the 
graphical device, for example by running `?svg`. 

### Cytoscape

Saving visualizations for Cytoscape works largely the same as for igraph. The
options used with 'vis_export_opts' must be compatible with `RCy3::exportImage`. 
To learn which export options are available, check the documentation with 
`?RCy3::exportImage`. 
...
if (cyto_active) {
    anvis(sepsis_net, 
          ...
          vis_save = T, 
          vis_export_type = "png", 
          vis_export_opts = list(resolution = 300))
}
```

```{r, echo=FALSE, fig.cap="Resulting image saved as png", out.width = '100%'}
knitr::include_graphics("cyto_images/image_saving.png")
```

In addition to exports of images, we can save and close the active cytoscape 
session. For this, seperate arguments are available with `anvis()`:
'cyto_save_session', 'cyto_close_session', which can be set to `TRUE` or `FALSE`. 
By default, the sessions are not saved and the session is closed if an image is
exported. 

# Exporting the networks

To use networks modified with `anvis` in different software the 'output_type' 
of `anvis()` can be set to "network". With this setting, networks can be saved 
in common formats such as "XGMML", "table", "sif", "tab", "tgf", "net". We show
network exports with two different file types below.

To export create a network for exporting create a network from an adjacency 
matrix with `adjToNetwork`, or add attributes to an existing network with 
`addVisAttrs`.

Creating a network from an adjacency matrix with visual attributes automatically
inferred by `anvis`. 

```{r}
# Create grouping vector for the nodes of the network
proteins <- colnames(sepsis[[1]])
groups <- dplyr::case_when(
    stringr::str_starts(proteins, "IL") ~ "group A", 
    stringr::str_starts(proteins, "CCL") ~ "group B", 
...
```{r, eval=FALSE}
# Note: xgmml files have an additional network title that we can choose
anvis(sepsis_net,
      output_type = "network", 
      netw_ext = "XGMML", 
      netw_xgmml_title = "Hospital patients")
```

Saving a list of networks as "table" with custom names:

```{r, eval=FALSE}
# Save all our networks as tab separated edge table and node table
anvis(net_list,
      output_type = "network", 
      netw_ext = "table",
      save_names = names(net_list))
```
