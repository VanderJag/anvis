...
network_for_visualisation <- PCLRC.gmm(datamatrix, prob.threshold = 0.95, Niter=1000, frac=0.75, rank.thr=0.3)

# Visualization to compare with the other dataset

groups <- rep("A", nrow(network_for_visualisation$CorrMatFiltered))
idx_ccl <- which(str_detect(colnames(network_for_visualisation$CorrMatFiltered), "^CCL"))
idx_il <- which(str_detect(colnames(network_for_visualisation$CorrMatFiltered), "^IL"))
idx_cxcl <- which(str_detect(colnames(network_for_visualisation$CorrMatFiltered), "^CXCL"))
groups[idx_il] <- "B"
groups[idx_ccl] <- "C"
groups[idx_cxcl] <- "D"

network_for_visualisation$CorrMatFiltered %>% VisualiseNetwork(node_attrs = "all", edge_attrs = "all", group_vec = groups)

STI <- network_for_visualisation$CorrMatFiltered

usethis::use_data(STI, overwrite = TRUE)
